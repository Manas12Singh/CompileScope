...
static void json_escape(FILE *fp, const char *s, int len) {
  for (int i = 0; i < len; i++) {
    char c = s[i];
    switch (c) {
      case '\"': fputs("\\\"", fp); break;
...
// Forward declaration
static void ast_to_json(FILE *fp, Node *node, int indent);
...
static void ast_to_json(FILE *fp, Node *node, int indent) {
  ...
  fprintf(fp, "\"kind\": %d", node->kind);

  if (node->tok && node->tok->loc) {
    fputs(",\n", fp);
    print_indent(fp, next);
    fputs("\"token\": \"", fp);
    json_escape(fp, node->tok->loc, node->tok->len);
    fputs("\"", fp);
  }
  if (node->val) {
    fputs(",\n", fp);
    print_indent(fp, next);
    fprintf(fp, "\"val\": %ld", (long)node->val);
  }
  if (node->fval) {
    fputs(",\n", fp);
    print_indent(fp, next);
    fprintf(fp, "\"fval\": %lf", (double)node->fval);
  }
  if (node->label) {
    fputs(",\n", fp);
    print_indent(fp, next);
    fprintf(fp, "\"label\": \"%s\"", node->label);
  }
  if (node->unique_label) {
    fputs(",\n", fp);
    print_indent(fp, next);
    fprintf(fp, "\"unique_label\": \"%s\"", node->unique_label);
  }
  ...
  if (node->lhs) {
    ...
    print_indent(fp, next);
    fputs("\"lhs\": ", fp);
    ast_to_json(fp, node->lhs, next);
  }
  if (node->rhs) {
    fputs(",\n", fp);
    print_indent(fp, next);
    fputs("\"rhs\": ", fp);
    ast_to_json(fp, node->rhs, next);
  }
  if (node->cond) {
    fputs(",\n", fp);
    print_indent(fp, next);
    fputs("\"cond\": ", fp);
    ast_to_json(fp, node->cond, next);
  }
  if (node->then) {
    fputs(",\n", fp);
    print_indent(fp, next);
    fputs("\"then\": ", fp);
    ast_to_json(fp, node->then, next);
  }
  if (node->els) {
    fputs(",\n", fp);
    print_indent(fp, next);
...
// Store the AST to a JSON file
void store_ast_to_json(const char *filename, Obj *globals) {
