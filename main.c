...
static void cc1(void) {
  ...
  // Save tokens to a txt file (kind name and loc only)
  FILE *tokfile = fopen("tokens.txt", "w");
  if (!tokfile)
    error("cannot open tokens.txt: %s", strerror(errno));
  for (Token *t = tok; t && t->kind != TK_EOF; t = t->next) {
    const char *kind_name;
    switch (t->kind) {
      case TK_IDENT: kind_name = "TK_IDENT"; break;
      case TK_PUNCT: kind_name = "TK_PUNCT"; break;
      case TK_KEYWORD: kind_name = "TK_KEYWORD"; break;
      case TK_STR: kind_name = "TK_STR"; break;
      case TK_NUM: kind_name = "TK_NUM"; break;
      case TK_EOF: kind_name = "TK_EOF"; break;
      default: kind_name = "UNKNOWN"; break;
    }
    fprintf(tokfile, "kind=%s, loc='%.*s'\n", kind_name, t->len, t->loc);
  }
  fclose(tokfile);
  ...
  Obj *prog = parse(tok);
  store_ast_to_json("ast.json", prog);
